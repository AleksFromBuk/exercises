 ...
 * Для этого есть перегруженный метод reduce(). Первый аргумент начальное
 * значение, второй оператор. В отличие от первого метода, который принимает
 * один аргумент, этот метод возвращает начальное значение для пустого списка,
 * а не Optional.
 *
